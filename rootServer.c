...
int main(int argc, char *argv[])
{
	...
	const char* local_server_IP = "127.0.0.3";
    ...
    for(;;){
    	...
    	unsigned char* string = ReadName(name, buf, &stop); // write information to new packet
		...
		if(strcmp(tail, "com") == 0)// match the top level domain with server name
		{
			strcpy(server_or_host, "comorgServer");
		}
		else if(strcmp(tail, "org") == 0)
		{
			strcpy(server_or_host, "comorgServer");
		}
		else if(strcmp(tail, "cn") == 0)
		{
			strcpy(server_or_host, "cnusServer");
		}
		else if(strcmp(tail, "us") == 0)
		{
			strcpy(server_or_host, "cnusServer");
		}
		else
		...
		memset(duplicate_buf,0,sizeof(duplicate_buf));
		client_qname_duplicate = (unsigned char*)&duplicate_buf;
		...
		authority_header->type = htons(T_NS);//NS type
    	authority_header->_class = htons(1);
    	authority_header->ttl = htons(100);
    	authority_header->data_len = htons(0);

		unsigned char* server_or_host_duplicate; // duplicate the previous server_or_host string
		unsigned char duplicate_buf2[40]; // used to store the name of server
    	...
    	ChangetoDnsNameFormat(domain_name, server_or_host); // write the server name to buffer
		...
		if(strcmp(tail, "com") == 0) // map the server name to IP address
		{
			memset(answer, 127, 1); answer++;
			memset(answer, 0, 1); answer++;
    		memset(answer, 0, 1); answer++;
			memset(answer, 4, 1); answer++;
		}
		else if(strcmp(tail, "org") == 0)
		{
			memset(answer, 127, 1); answer++;
